...
//                pw.println("    protected Object set(Field field, Object p0) { return values.put(field, p0); }");
                ...
                pw.println("        return " + (hasId ? "(getId() == null ? super.hashCode() : getId().hashCode())" : "super.hashCode()") + ";");
                pw.println("    }");
                ...
                methodsPerAttribute.entrySet().stream().sorted(Comparator.comparing(Map.Entry::getKey)).forEach(me -> {
                    ...
                    for (ExecutableElement method : methods) {
                        ...
                        if (fat != FieldAccessorType.UNKNOWN && ! printMethodBody(pw, fat, method, "f" + me.getKey(), fieldType)) {
