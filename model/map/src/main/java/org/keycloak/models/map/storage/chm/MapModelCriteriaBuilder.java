...
public class MapModelCriteriaBuilder<K, V extends AbstractEntity, M> implements ModelCriteriaBuilder<M, MapModelCriteriaBuilder<K, V, M>>, Predicate<V> {
    ...
    public final MapModelCriteriaBuilder<K, V, M> and(MapModelCriteriaBuilder<K, V, M>... builders) {
        ...
        return instantiateNewInstance(resIndexFilter, resEntityFilter);
    }
    ...
    protected MapModelCriteriaBuilder<K, V, M> idCompare(Operator op, Object[] values) {
        ...
        switch (op) {
            ...
            case NOT_EXISTS:
            case IN:
                return instantiateNewInstance(this.keyFilter.and(CriteriaOperator.predicateFor(op, convertedValues)), this.entityFilter);
            default:
