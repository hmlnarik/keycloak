...
public class DeepCloner {
    ...
    public static class Builder {
        ...
        public <V> Builder cloner(Class<? extends V> clazz, Cloner clonerWithId, Cloner clonerWithoutId) {
            ...
            if (clonerWithoutId != null) {
                forThisClassAndAllMarkedParentsAndInterfaces(clazz, cl -> this.clonersWithId.put(cl, clonerWithoutId));
            }
            return this;
        }
     ...
     * @return Instance which has all the properties same as the {@code from}. Preferrably, {@code to} is returned.
    ...
    private <V> V deepClone(V from, V to, Map<Class<?>, Cloner> cloners) {
        ...
        if (cloner == null) {
            cloner = (Cloner<V>) genericCloner;
            LOG.debugf("Using generic cloner for %s", from.getClass());
        }
        return cloner == null ? warnCloneNotSupported(from) : cloner.clone(from, to);
    }
    ...
    /**
     * Creates a new instance of the given type and copies its properties including ID from the {@code from} instance
     * @param <V> Type (class or a {@code @Root} interface) to create a new instance and clone properties from
     * @param from Original instance
     * @return Newly created instance or {@code null} if {@code from} is {@code null}.
     */
    public <V extends AbstractEntity> V from(V from) {
        return from == null ? null : from(from.getId(), from);
    }
