...
|Do Not Store Users
|The identity provider does not store users in the {project_name} database. When *ON*, {project_name} does not store users from this identity provider in the {project_name} database, and the user is removed upon logout. See <<_transient-users, Transient Users for Brokering>> for more information.
