...
public abstract class AbstractCrossDCTest extends AbstractTestRealmKeycloakTest {
    ...
    public void enableOnlyFirstNodeInFirstDc() {
        for (int i = 0; i < suiteContext.getDcAuthServerBackendsInfo().size(); i ++) {
            disableDcOnLoadBalancer(i);
        }
        getAutomaticallyStartedBackendNodes(0)
          .findFirst()
          .ifPresent(c -> loadBalancerCtrl.enableBackendNodeByName(c.getQualifier()));
    }

    private void enableAllButFirstDc() {
        for (int i = 1; i < suiteContext.getDcAuthServerBackendsInfo().size(); i ++) {
            enableDcOnLoadBalancer(i);
        }
    }
    ...
    public void initLoadBalancer() {
        ...
        this.suiteContext.getDcAuthServerBackendsInfo().forEach(
          cs -> cs.stream()
